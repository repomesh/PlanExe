...
_SECTION_GUIDANCE = {
    ReportSectionTypeEnum.SELECTED_SCENARIO.value: (
        ...
        "baseline scenario. Rejected alternatives ('Pioneer's Edge', 'Tourist Premium', "
        "any scenario the plan did NOT pick) must not produce numeric_values, "
        "load_bearing_assumptions, gates_and_thresholds, or risks_and_shocks. A rejected "
        "alternative may be named in source_quote ONLY for disambiguation. Do NOT write "
        "gates of the form 'If the <rejected> scenario were chosen, then ...' — that is "
        "modelling the wrong plan. Do NOT include numbers that quantify only a rejected "
        "alternative (e.g. 'zero contingency under the high-risk option'). "
