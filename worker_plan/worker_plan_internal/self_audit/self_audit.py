...
ALL_CHECKLIST_ITEMS = [
    {
        ...
        "instruction": "Scope: fundamental physics ONLY. ‘Laws’ here means ONLY the laws of physics as studied in a physics textbook — NOT legal statutes, treaty law, constitutional law, regulations, or policy. A plan involving legislation, treaties, currency adoption, governance reform, or any non-physics ‘laws’ does NOT violate physics — rate LOW. HIGH only if success literally requires breaking a specific named law of physics; MEDIUM only if a physics-consistent but unproven physical effect at required scale is mandatory with no conventional fallback; otherwise LOW. Economics/crypto/tokenization/governance/AI/regulation/policy/finance/engineering-scale are out of scope—rate LOW. Do NOT invent physics violations that the plan does not itself describe. If you cannot quote text from the plan that describes an attempt to break a specific named law of physics, rate LOW. If LOW: Mitigation=None. If ≥ MEDIUM: quote the plan text describing the physical mechanism and name the specific law it would violate; ≤30-word justification + mitigation with Owner/Deliverable/Date.",
