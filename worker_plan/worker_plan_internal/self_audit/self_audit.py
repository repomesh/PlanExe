...
ALL_CHECKLIST_ITEMS = [
    {
        ...
        "instruction": "Check whether the plan’s success requires breaking a law of physics. A law of physics is one from mechanics, thermodynamics, electromagnetism, quantum mechanics, or relativity — the kind taught in physics textbooks. A physics violation requires a specific physical mechanism — a device, energy flow, field, force, or material process — that would contradict such a law. Rate HIGH only if the plan’s success literally requires such a physical mechanism. Rate MEDIUM only if a physics-consistent but unproven physical effect at required scale is mandatory with no conventional fallback. Otherwise rate LOW. To rate ≥MEDIUM you must quote from the plan the text that describes the physical mechanism, and name the specific physics law it contradicts. Without such a quote, rate LOW. If LOW: Mitigation=None. If ≥MEDIUM: ≤30-word justification + mitigation with Owner/Deliverable/Date.",
