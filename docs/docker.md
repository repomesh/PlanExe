...
- Shared volumes: `.env` and `./llm_config/` are mounted read-only. Ensure they exist on the host before starting. Each container writes run outputs to its own `/app/run`.
