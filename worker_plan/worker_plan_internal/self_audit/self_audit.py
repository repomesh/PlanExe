...
ALL_CHECKLIST_ITEMS = [
    {
        ...
        "instruction": "Check whether the plan’s success requires breaking a law of physics. A law of physics is one from mechanics, thermodynamics, electromagnetism, quantum mechanics, or relativity — the kind taught in physics textbooks and expressible in SI units (joules, newtons, kelvin, coulombs, meters per second, and so on). Rate HIGH only if the plan’s success literally requires breaking such a law. Rate MEDIUM only if a physics-consistent but unproven physical effect at required scale is mandatory with no conventional fallback. Otherwise rate LOW. To rate ≥MEDIUM the justification must contain ALL THREE of: (a) a verbatim quote from the plan text describing the proposed physical process; (b) the name of the specific physics law that is contradicted; (c) the specific physical quantity in SI units that would be violated, with a numerical magnitude. If any of (a), (b), (c) is not available from the plan text, rate LOW. If LOW: Mitigation=None. If ≥MEDIUM: ≤30-word justification + mitigation with Owner/Deliverable/Date.",
