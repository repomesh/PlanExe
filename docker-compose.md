...
- Shared host files: `.env` and `llm_config.<profile>.json` mounted read-only; `./run` bind-mounted so outputs persist; `.env` is also loaded via `env_file`.
...
- Ensure `.env` and `llm_config.<profile>.json` exist; copy `.env.docker-example` to `.env` if you need a starter.
...
- Volumes: mirrors the worker (`.env` ro, `llm_config.<profile>.json` ro, `run/` rw) so both share config and outputs.
...
- Volumes: `.env` (ro), `llm_config.<profile>.json` (ro), `run/` (rw).
...
- Volumes: `.env` (ro), `llm_config.<profile>.json` (ro), `run/` (rw for pipeline output).
...
- `.env` must exist before `docker compose up`; it is both loaded and mounted read-only. Same for `llm_config.<profile>.json`. If missing, start from `.env.docker-example`.
