...
ALL_CHECKLIST_ITEMS = [
    {
        ...
        "instruction": "This check exists to catch plans whose premise is fantasy or fictional-universe physics — prompts that read like a story set in a magical world or that require a mechanic the real world cannot offer (e.g., magic, time travel, faster-than-light travel, perpetual motion, reactionless propulsion, summoning, teleportation by spell, resurrection). Default rating: LOW. Rate HIGH only when the plan's success literally depends on breaking a named law of physics (e.g., second law of thermodynamics, conservation of energy, speed-of-light limit, causality) — name the specific law in the justification. HARD GATE: if you cannot name a specific law of physics that the plan would have to break, the rating MUST be LOW. Missing details, undefined parameters, unspecified strategies, vague deliverables, or items the plan flags as ‘Missing Information’ are NEVER physics violations on their own; route those gaps to other checklist items (engineering depth, evidence, deliverable clarity). Real-world engineering, naturally occurring materials (including radioisotopes), novel-but-plausible technology, regulatory/permitting/licensing/handling gaps, linguistic/social/policy design, ambitious budgets, and tight timelines all stay LOW; the legal/regulatory check covers authorization gaps. LOW-mitigation TEMPLATE: when level is LOW (the common case), the mitigation MUST stay on the physics-violation topic and read along the lines of 'Project Manager: During scope reviews, confirm no fantasy-physics dependency (magic, time travel, FTL, perpetual motion) has been introduced — no further action required.' Do NOT borrow unrelated tasks from elsewhere in the plan (governance, finance, linguistic detail, legal review, etc.) — those belong to other checklist items, not this one. If ≥MEDIUM: ≤30-word justification + mitigation with Owner/Deliverable/Timeframe.",
        "comment": "Intent: catch prompts whose setting is a fantasy / fictional universe (magic, time travel, Hogwarts/Middle-earth-style worlds, FTL, summoning, resurrection). Real-world plans — even ambitious, expensive, or regulated ones — should default LOW. Observed false-positive modes worth defending against: (1) Confusing legal/regulatory ‘laws’ with ‘laws of physics’ for plans about policy, currency adoption, governance, etc. (2) Treating an authorization/permit/licensing/handling gap as a physics violation. Concrete example seen on main: a plan involving a Cf-252 isotopic source rated HIGH because ‘handling radioactive material requires NRC authorization’ — that is regulatory (item 12), not physics; Cf-252 itself exists under known physics. (3) An ‘If LOW: Mitigation=None.’ carve-out in this per-checklist instruction once produced ‘Mitigation: N/A’ outputs, contradicting the global STRICT RULE that mitigation must never be N/A and on LOW should reinforce good practice; the carve-out has been removed and the global rule now drives LOW behaviour. (4) Reading the subtitle's old ‘fundamental science’ phrasing as ‘any unspecified fundamental of the plan’ — a linguistics plan (‘Clear English’ standard) was rated HIGH because the plan listed ‘Detailed Grapheme-to-Phoneme mapping strategy’ as Missing Information; the model interpreted any missing ‘fundamental’ detail as a physics gap. Subtitle has been retargeted to physics directly, and the instruction now hard-gates HIGH on naming a specific physics law — missing details on their own can never trigger HIGH."
    },
    {
        "index": 2,
