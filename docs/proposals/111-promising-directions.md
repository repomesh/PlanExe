...
| **114-I2** | Failure Diagnostics in `plan_status` | ✅ **Implemented**. Four DB columns (`failure_reason`, `failed_step`, `error_message`, `recoverable`) populated by worker on failure and surfaced in `plan_status` inside a consolidated `error` dict. Agents can now explain why a plan failed and recommend resume vs retry. Diagnostics reset on retry/resume |
