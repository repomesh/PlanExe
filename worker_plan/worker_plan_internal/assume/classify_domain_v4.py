...
v4: tightens the system prompt with a "specificity rule" so the model
prefers narrow expert disciplines (Astrophysics, Drug Discovery,
Renewable Energy, Machine Learning) over broad umbrellas (Research,
Energy, Engineering, Science) whenever the prompt contains signals
for a specific subfield. v3 was the prior baseline.

PROMPT> python -m worker_plan_internal.assume.classify_domain_v4
"""
import time
import logging
import json
...
- Niche labels (Neuroscience, Biomedicine, Machine Learning,
  ...
  schema lets them.
- v4 specificity rule: broad umbrella labels — Research,
  Engineering, Science, Technology, Business, Industry, Energy —
  must NOT be the primary domain when the prompt contains signals
  for a more specific discipline. The fix is a positive substitute
  (worked examples showing exoplanet transits → Astrophysics, mRNA
  → Drug Discovery, wind farm → Renewable Energy, transformer
  fine-tuning → Machine Learning) rather than a "do not use
  Research" rule. The umbrella is acceptable as a secondary
  (role="method"/"market") or as the fallback primary only when
  no subfield can be inferred.
...
Worked examples of the descent from broad to narrow:
- A prompt about exoplanet transit photometry, gravitational-wave detection, or measurements of a specific cosmological observable → primary "Astrophysics"; secondaries "Scientific Instrumentation" (method), "Data Analysis" (method) where relevant. NOT "Research", NOT "Science".
- A prompt about mRNA vaccines, monoclonal antibodies, CRISPR gene editing, or small-molecule drug discovery → "Drug Discovery", "Biotechnology", "Genetics", or "Immunology" — whichever fits. NOT "Healthcare", NOT "Research".
- A prompt about a wind farm, solar farm, geothermal plant, or grid-scale battery → "Renewable Energy" or a more specific subfield (Wind Energy, Solar Energy, Battery Technology). NOT "Energy", NOT "Construction".
- A prompt about a transformer language model, fine-tuning, embeddings, or RLHF → "Machine Learning". NOT "Software" alone.
- A prompt about a real-time event filter or trigger logic for a sensor pipeline → "Signal Processing" or "Data Acquisition". NOT "Software", NOT "Research".
- A prompt about a clinical trial of a specific therapy → the therapy's discipline (e.g. "Oncology", "Cellular Therapy") with secondary "Clinical Research". NOT "Research" as primary.
- A prompt naming a generic "scientific study" with no identifiable subfield, no named instrument, no named substance, no named technique → "Research" is the right fallback primary.

If multiple narrow disciplines all fit (e.g. a project that is equally Drug Discovery and Genetics), pick the one that names the project's main success criterion as the primary outcome and put the others as method/constraint secondaries.
...
- scientific study / paper / experiment → use the named subfield as the primary outcome (Astrophysics, Neuroscience, Genetics, Drug Discovery, Marine Biology, Volcanology, ...) and "Research" only as the fallback when the prompt names no identifiable subfield, no specific instrument, no specific substance, and no specific technique. If a publishable finding is the deliverable, "Research" can appear as a secondary with role="outcome", but the named subfield should be the primary.
