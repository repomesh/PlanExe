...
class TestFindStageByFilename(unittest.TestCase):
    def test_find_report(self):
        stage = find_node_by_filename("030-report.html")
        self.assertIsNotNone(stage)
        self.assertEqual(node.name, "report")

    def test_find_potential_levers_clean(self):
        stage = find_node_by_filename("002-10-potential_levers.json")
        self.assertIsNotNone(stage)
        self.assertEqual(node.name, "potential_levers")

    def test_find_potential_levers_raw(self):
        stage = find_node_by_filename("002-9-potential_levers_raw.json")
        self.assertIsNotNone(stage)
        self.assertEqual(node.name, "potential_levers")

    def test_find_executive_summary(self):
        stage = find_node_by_filename("025-2-executive_summary.md")
        self.assertIsNotNone(stage)
...
#     inputs=(NodeInput(from_node="setup", artifact_path="001-2-plan.txt"), ...),
