...
2. When several candidates share `role="outcome"`, prefer the narrowest discipline (a specialist subfield over an umbrella). The same narrowness principle the upstream classifier uses.
...
def _format_candidate_list(fits: list[DomainFit]) -> str:
    ...
    for i, f in enumerate(fits):
        lines.append(
            f"- `[{i}]` domain={f.domain!r}, fit={f.fit!r}, "
...
class ClassifyDomain:
    ...
    def execute(
        ...
        primary_llm: optional separate LLM used for the v7 second-pass
